...
public class GXDLMSBase extends GXDLMSSecureServer
    ...
    public Properties pDLMS2DSAssociation;
    public Properties pDS2DLMSAssociation;
    public Properties pDataSet = null;
    public String sPrefix = "";
    public int iPeriodicUpdate;
    public Vector<GXDLMSObject> vProfiles = new Vector<GXDLMSObject>();
    public Vector<Long> vProfilesPeriods = new Vector<Long>();
    public Vector<Long> vProfilesLastUpdate = new Vector<Long>();

    public void LoadObjects(String sObjFileName) {
        try {
            GXDLMSObjectCollection gxObjectCollection = GuruxUtil.GXDLMSObjectLoadWithVals(sObjFileName);

            for (GXDLMSObject it : gxObjectCollection) {
                if (it.getObjectType() == ObjectType.PROFILE_GENERIC) {
                    if (((GXDLMSProfileGeneric) it).getCapturePeriod() > 0) {
                        vProfiles.add(it);
                        vProfilesLastUpdate.add(new Long((new Date()).getTime() / 1000));
                        vProfilesPeriods.add(new Long(((GXDLMSProfileGeneric) it).getCapturePeriod()));
                        ((GXDLMSProfileGeneric) it).setCapturePeriod(0);
                    }
                }
                getItems().add(it);
            }
    ...
    public void UpdateLoadProfiles() {
        int i;
        long lNow = ((new Date()).getTime() / 1000);

        for (i = 0; i < vProfiles.size(); i++) {
            try {
                if ((lNow % vProfilesPeriods.elementAt(i) == 0) &&
                        (vProfilesLastUpdate.elementAt(i).longValue()!=lNow)){
                    //System.out.println(sdf.format(new Date(lNow*1000)));
                    vProfilesLastUpdate.set(i, lNow);
                    Update();
                    ((GXDLMSProfileGeneric) vProfiles.elementAt(i)).capture(this);
                }
            } catch (Exception ex) {
    ...
    public void Update() {
        ...
        String sKey = "";
        String sValue = "";
        Enumeration sKeys = pDLMS2DSAssociation.keys();
        GXDLMSObject it;
        while (sKeys.hasMoreElements()) {
            try {
                sLN = (String) sKeys.nextElement();
                sKey = sPrefix + (String) pDLMS2DSAssociation.get(sLN);
                sValue = (String) pDataSet.get(sKey);
                if (sValue == null) {
                    continue;
                }
                it = this.getItems().findByLN(ObjectType.ALL, sLN);
                if (it == null) {
                    continue;
                }
                GuruxUtil.GXDLMSObjectSetValue(it, sValue);
            } catch (Exception ex) {
            }
        }
    }
    ...
    public void read(ValueEventArgs[] args) {
        ...
        for (ValueEventArgs e : args) {
            ...
            if (e.getTarget() instanceof GXDLMSAssociationLogicalName
                    ...
                    || e.getTarget() instanceof GXDLMSProfileGeneric) // Framework will handle profile generic automatically.
            {
                continue;
            }
            System.out.println(
